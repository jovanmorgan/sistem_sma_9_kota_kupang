...
var dataGuru = [
    { foto: "foto_guru1.jpg", nama: "John Doe", nip: "1234567890", alamat: "Jl. Contoh No. 123", telepon: "081234567890" },
    // Data Guru lainnya bisa ditambahkan di sini
];

// Fungsi untuk menampilkan data guru berikutnya
function nextData() {
    if (currentDataIndex < dataGuru.length - 1) {
        currentDataIndex++;
    } else {
        currentDataIndex = 0;
    }
...
function displayData() {
    var tableBody = document.getElementById("dataGuru").getElementsByTagName("tbody")[0];
    var newRow = tableBody.insertRow();

    newRow.innerHTML = `
            <td><img src="${dataGuru[currentDataIndex].foto}" alt="Foto Guru"></td>
            <td>${dataGuru[currentDataIndex].nama}</td>
            <td>${dataGuru[currentDataIndex].nip}</td>
            <td>${dataGuru[currentDataIndex].alamat}</td>
            <td>${dataGuru[currentDataIndex].telepon}</td>
        `;
}
